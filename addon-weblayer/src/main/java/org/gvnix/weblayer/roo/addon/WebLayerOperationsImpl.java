...
// container
@Service
public class WebLayerOperationsImpl implements WebLayerOperations {

    private static final Logger logger = HandlerUtils
            .getLogger(WebLayerOperationsImpl.class);

    private static final JavaType GVNIX_WEBLAYER_SHOW = new JavaType(
            GvNIXWebLayer.class.getName());
    ...
    public void annotateType(JavaType javaType) {
        ...
        if (MemberFindingUtils.getAnnotationOfType(
                mutableTypeDetails.getAnnotations(), GVNIX_WEBLAYER_SHOW) == null) {

            // Create JavaType instance for the add-ons trigger annotation
            JavaType rooRooAddon = GVNIX_WEBLAYER_SHOW;
    ...
    public void setup() {
        ...
        // annotation
        projectOperations
                .addRepository(new Repository("Addon Roo add-on repository",
                        "Addon Roo add-on repository",
                        "https://org-gvnix-weblayer-roo-addon.googlecode.com/svn/repo"));

        List<Dependency> dependencies = new ArrayList<Dependency>();

        // Install the dependency on the add-on jar (
        dependencies.add(new Dependency("org.gvnix.weblayer.roo.addon",
                "org.gvnix.weblayer.roo.addon", "0.1.0.BUILD-SNAPSHOT",
    ...
    public void scaffoldShow(JavaType entity, JavaPackage viewPackage) {
        // Use Roo's Assert type for null checks
        Assert.notNull(entity, "Entity required");

        // Retrieve metadata for the Java source type the annotation is being
        // added to
        String id = physicalTypeMetadataProvider.findIdentifier(entity);
        if (id == null) {
            throw new IllegalArgumentException("Cannot locate source for '"
        ...
        // Test if the annotation already exists on the target type
        boolean alreadyAnnotated = (MemberFindingUtils.getAnnotationOfType(
                mutableTypeDetails.getAnnotations(), GVNIX_WEBLAYER_SHOW) == null);
        if (!alreadyAnnotated) {

            // Create JavaType instance for the add-ons trigger annotation
            JavaType webLayerShowAnnotation = GVNIX_WEBLAYER_SHOW;

            // Create Annotation metadata
            List<AnnotationAttributeValue<?>> annotationAttrs = new ArrayList<AnnotationAttributeValue<?>>();
            List<StringAttributeValue> attrValList = new ArrayList<StringAttributeValue>();
            StringAttributeValue viewPackageAttrValue = new StringAttributeValue(
                    new JavaSymbolName("__ARRAY_ELEMENT__"),
                    viewPackage.getFullyQualifiedPackageName());
            attrValList.add(viewPackageAttrValue);
            annotationAttrs.add(new ArrayAttributeValue<StringAttributeValue>(
                    new JavaSymbolName("viewPackage"), attrValList));
            AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(
                    webLayerShowAnnotation, annotationAttrs);

            // Add annotation to target type
            mutableTypeDetails.addTypeAnnotation(annotationBuilder.build());
        } else {
            logger.warning(getAlreadyAnnotatedErrorMsg());
        }
    }

    private String getAlreadyAnnotatedErrorMsg() {
        return "Entity is alreadyAnnotated with @".concat(GVNIX_WEBLAYER_SHOW
