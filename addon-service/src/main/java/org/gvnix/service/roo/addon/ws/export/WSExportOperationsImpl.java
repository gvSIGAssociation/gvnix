...
@Component
@Service
public class WSExportOperationsImpl implements
        WSExportOperations {

    private static Logger logger = Logger
            .getLogger(WSExportOperations.class.getName());
  ...
  public void exportService(JavaType serviceClass, String serviceName,
    ...
    if (!fileManager.exists(fileLocation)) {
      logger.log(
          Level.INFO,
          "Crea la nueva clase de servicio: "
    ...
    List<AnnotationAttributeValue<?>> gvNixAnnotationAttributes = exportServiceAnnotationAttributes(
        serviceClass, serviceName, portTypeName, targetNamespace, addressName);

    annotationsService.addJavaTypeAnnotation(serviceClass,
        GvNIXWebService.class.getName(), gvNixAnnotationAttributes, false);
   ...
   */
  private AnnotationMetadata getDefaultGvNIXWebServiceAnnotation(JavaType serviceClass) {
    ...
    String targetNamespace = wSConfigService
        .convertPackageToTargetNamespace(serviceClass.getPackage().toString());
   ...
   * {@inheritDoc}
   */
  public List<AnnotationAttributeValue<?>> exportServiceAnnotationAttributes(JavaType serviceClass,
                                                                             String serviceName,
                                                                             String portTypeName,
                                                                             String targetNamespace,
                                                                             String addressName) {
    // Checks serviceName parameter to publish the web service.
    serviceName = StringUtils.hasText(serviceName) ? serviceName : serviceClass
        .getSimpleTypeName();
    ...
    gvNixAnnotationAttributes.add(new StringAttributeValue(new JavaSymbolName(
        "name"), portTypeName));
    gvNixAnnotationAttributes.add(new StringAttributeValue(new JavaSymbolName(
    ...
    gvNixAnnotationAttributes.add(new StringAttributeValue(new JavaSymbolName(
        "address"), addressName));
    gvNixAnnotationAttributes.add(new StringAttributeValue(new JavaSymbolName(
        "fullyQualifiedTypeName"), serviceClass.getFullyQualifiedTypeName()));
    gvNixAnnotationAttributes.add(new BooleanAttributeValue(new JavaSymbolName(
        "exported"), false));
   ...
   * {@inheritDoc}
   */
  public void exportOperation(JavaType serviceClass, JavaSymbolName methodName,
                              String operationName, String resultName,
    ...
    wSExportValidationService.checkAuthorizedJavaTypesInOperation(serviceClass,
        methodName);
    ...
    wSExportValidationService.checkMethodExceptions(serviceClass, methodName,
    ...
    if (!isReturnTypeVoid) {
      ...
      Assert
          .isTrue(
              wSExportValidationService.checkNamespaceFormat(resultNamespace),
    ...
    List<AnnotationMetadata> annotationMetadataUpdateList = getAnnotationsToExportOperation(
        ...
        requestWrapperName, requestWrapperNamespace, webServiceTargetNamespace);
   ...
   * 
   * @param serviceClass where the method is defined.
  ...
  private JavaType returnJavaType(JavaType serviceClass,
    ...
    MethodMetadata methodMetadata = javaParserService.getMethodByNameInClass(
   ...
   * </ul>
   */
  public List<AnnotationMetadata> getAnnotationsToExportOperation(JavaType serviceClass,
                                                                  JavaSymbolName methodName,
    ...
    if ((resultName != null && returnType != null)
      ...
      responseWrapperNamespace = StringUtils.hasText(responseWrapperNamespace) ? responseWrapperNamespace
          : webServiceTargetNamespace;
      ...
      ClassAttributeValue resultTypeAttributeValue = new ClassAttributeValue(
          new JavaSymbolName("webResultType"), JavaType.VOID_PRIMITIVE);
      annotationAttributeValueList.add(resultTypeAttributeValue);
    }
  ...
  private boolean isWebServiceClass(JavaType serviceClass) {
    ...
    Assert.notNull(id,
        "Cannot locate source for '" + serviceClass.getFullyQualifiedTypeName()
            + "'.");
    ...
    String entityMid = WSExportMetadata.createIdentifier(serviceClass, path);
  ...
  public boolean isMethodAvailableToExport(JavaType serviceClass,
    ...
    Assert.isTrue(exists == false, "The method '" + methodName
        + "' has been annotated with @" + annotationName
   ...
   * {@inheritDoc}
   */
  public List<AnnotatedJavaType> getMethodParameterAnnotations(JavaType serviceClass,
                                                               JavaSymbolName methodName,
                                                               String webServiceTargetNamespace) {
    ...
    for (AnnotatedJavaType parameterType : parameterTypesList) {
      ...
      AnnotationMetadata webParamAnnotationMetadata = new AnnotationMetadataBuilder(
          new JavaType("javax.jws.WebParam"), webParamAttributeValueList)
          .build();
  ...
  public String getAvailableServiceOperationsToExport(JavaType serviceClass) {
    ...
    for (MethodMetadata methodMetadata : methodList) {
      ...
      if (!isAnnotationIntroduced) {
        ...
        methodListStringBuilder.append("\t* "
            + methodMetadata.getMethodName().getSymbolName() + "\n");
      }
