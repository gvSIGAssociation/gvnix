...
@Component
@Service
public class WSConfigServiceImpl implements
        WSConfigService {
...
//    private MavenOperations mavenOperations;
  ...
  private void installDependencies(CommunicationSense type) {
    ...
    if (!isInstalled) {
      ...
      for (Element dependency : cxfDependencies) {
        ...
        // DiSiD: Unkown error when dependencyUpdate used, use addDependency
        // instead
    ...
    switch (type) {
      ...
      case IMPORT:
        ...
        projectProperties = XmlUtils.findElements(
            ...
            XmlUtils.getConfiguration(this.getClass(), "properties.xml"));
        break;
      ...
      case IMPORT_RPC_ENCODED:
        ...
        projectProperties = XmlUtils.findElements(
            "/configuration/gvnix/properties/*",
            XmlUtils.getConfiguration(this.getClass(), "properties-axis.xml"));
        break;
    }
   ...
   * <p>
   * <p>
   * Update cxf file if its necessary to avoid changes in WSDL contract checking
   * type annotation values from service class.
   * </p>
   * <p>
  ...
  public boolean exportClass(JavaType className,
    ...
    Assert.isTrue(annotationMetadata != null, "Annotation '"
        + annotationMetadata.getAnnotationType().getFullyQualifiedTypeName()
        + "' in class '" + className.getFullyQualifiedTypeName()
        + "'must not be null to check cxf xml configuration file.");

    // Update web service configuration file.
    boolean updateGvNIXWebServiceAnnotation = updateConfiguration(className,
        annotationMetadata);
  ...
  private boolean updateConfiguration(JavaType className,
    ...
    Assert.isTrue(
        serviceName != null && StringUtils.hasText(serviceName.getValue()),
    ...
    Element classAndIdService = XmlUtils.findFirstElement(
        "/beans/bean[@id='" + serviceName.getValue().concat("Impl")
            + "' and @class='" + className.getFullyQualifiedTypeName() + "']",
        root);
      ...
      if (updateFullyQualifiedTypeName) {
        ...
        if (classService != null) {
          ...
          if (classService != null) {
            ...
            if (!StringUtils.hasText(idValue)
                || !idValue
                    .contentEquals(serviceName.getValue().concat("Impl"))) {
              updateClassService.setAttribute("id", serviceName.getValue()
                  .concat("Impl"));
    ...
    Element jaxwsBean = XmlUtils.findFirstElement("/beans/endpoint[@address='/"
        + address.getValue() + "' and @id='" + serviceName.getValue() + "']",
        root);
    ...
    if (updateEndpoint) {
      ...
      Element addressEndpoint = XmlUtils.findFirstElement(
          "/beans/endpoint[@address='/" + address.getValue() + "']", root);
      ...
      if (addressEndpoint != null) {
        ...
        if (!StringUtils.hasText(idAttribute)
          ...
          addressEndpoint.getParentNode().replaceChild(updateAddressEndpoint,
              addressEndpoint);
      ...
      Element idEndpoint = XmlUtils.findFirstElement("/beans/endpoint[@id='"
      ...
      if (addressEndpoint == null && idEndpoint == null) {
        ...
        endpoint.setAttribute("implementor", "#".concat(serviceName.getValue())
            .concat("Impl"));
   ...
   * </p>
   * <p>
   * Defines an execution for the serviceClass with the serviceName to generate
   * in maven compile goal.
   * </p>
   */
  public void jaxwsBuildPlugin(JavaType serviceClass, String serviceName,
        ...
        while (updateServiceExecution != null) {
          ...
          if (updateServiceExecution.getNodeName().contentEquals("className")) {
            ...
            logger.log(Level.INFO, "Wsdl generation with CXF plugin for '"
                + serviceName + " service, updated className attribute for '"
    ...
    Element oldExecution = XmlUtils.findFirstElement(
        "/project/build/plugins/plugin/executions/execution[id='" + executionID
            + "']", root);
  ...
  public void installJaxwsBuildPlugin() {
    Element pluginElement = XmlUtils.findFirstElement("/jaxws-plugin/plugin",
   ...
   * {@inheritDoc}
   * <p>
   * Adds a wsdl location to the plugin configuration. If code generation plugin
   * configuration not exists, it will be created.
   * </p>
   */
  ...
  private void addExportWSDLLocationDocument(String wsdlLocation,
    ...
    Element plugin = XmlUtils.findFirstElement(
        "/cxf-codegen/cxf-codegen-plugin/plugin", XmlUtils.getConfiguration(
            this.getClass(), "dependencies-export-wsdl2java-plugin.xml"));
    ...
    if (oldGenerateSourcesCxfServer != null) {
      ...
      if (oldExecutions == null) {
        ...
        newExecutions = oldExecutions;
        newExecutions.appendChild(newGenerateSourcesCxfServer);
        oldExecutions.getParentNode()
            .replaceChild(newExecutions, oldExecutions);
      }
    }
  ...
  private boolean addImportLocationDocument(String wsdlLocation) {
    ...
    Element pluginTemplate = XmlUtils.findFirstElement(
        "/codegen-plugin/plugin", XmlUtils.getConfiguration(this.getClass(),
  ...
  public void mvn(String parameters, String message) throws IOException {
    ...
    File root = new File(projectOperations.getPathResolver().getRoot(Path.ROOT));
