...
public class WsdlParserUtilsTest {
	...
	root = wsdl.getDocumentElement();
	assertEquals("v2.emailvalidation.xwebemailvalidation.wsuxwebservicesucom.EmailValidation", WsdlParserUtils.getServiceClassPath(root));
	
	File file = new File(SRC_TEST_RESOURCES_PATH, TEMP_CONVERT_MODIFIED_LOCAL_WSDL);
	wsdl = XmlUtils.getDocumentBuilder().parse(file);
	root = wsdl.getDocumentElement();
	assertEquals("org.te3mupuuri.www.kk.idu1ur.TE_mP_002fC_0023ONe_0040R_002bT$GE_003dR_002aG_0027E_00282_00293_002c4_002f2_0025Rmm12Mm", WsdlParserUtils.getServiceClassPath(root));
    }
	...
	wsdl = XmlUtils.getDocumentBuilder().parse(EMAIL_VALIDATION_V2_WSDL);
	root = wsdl.getDocumentElement();
	assertEquals("v2.emailvalidation.xwebemailvalidation.wsuxwebservicesucom.XWebEmailValidationInterface", WsdlParserUtils.getPortTypeClassPath(root));
