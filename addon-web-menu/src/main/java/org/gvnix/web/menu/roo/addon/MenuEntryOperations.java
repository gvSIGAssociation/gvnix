...
public interface MenuEntryOperations {
   ...
   * 
   * @return true if the user installed the gvNIX menu, otherwise returns false.
   */
   ...
   * 
   * @return true if Spring Security 3.0.5 is installed. Otherwise returns false
   */
 ...
 void createWebArtefacts(String classesPackage);
  ...
  /**
   * Allows for the addition of menu categories and menu items. If a category or
   * menu item with the given identifier exists then it will <b>not</b> be
   ...
   * <p>
   * This method will <i>not</i> write i18n message codes. This means the caller
   * will manage the properties himself - allowing for better efficiency.
   * <p>
   ...
   * identifier to diffentiate between different categories provided by the same
   * addon. Similarly, the recommended menu item identifier naming convention is
   ...
   * @param link
   * @param idPrefix
   * @see org.springframework.roo.addon.web.mvc.jsp.menu.MenuOperations#addMenuItem(JavaSymbolName, JavaSymbolName, String, String, String)
   */
   ...
   * identifier to differentiate between different categories provided by the same
   * addon. Similarly, the recommended menu item identifier naming convention is
   ...
   * @param link
   * @param idPrefix
   * @see org.springframework.roo.addon.web.mvc.jsp.menu.MenuOperations#addMenuItem(JavaSymbolName, JavaSymbolName, String, String, String, String)
   */
  void addMenuItem(JavaSymbolName menuCategoryName, JavaSymbolName menuItemId,
   ...
   * authorized to access to given URL and a flag that indicates if the
   * menu entry should be visible in menu.
   * <p>
   * This method won't add neither Controller nor JSPs for new menu entries, so 
   * if you need them, add a new page using {@code controller class}
   * command. Menu add-on will detect the new controller all will add it
   * to menu.
   * <p>
   ...
   * <p>
   * Link is not required because gvNIX menu item could act as
   * sub-category. 
   * 
   * @param menuCategoryName
   * @param menuItemId
   ...
   * @see org.springframework.roo.addon.web.mvc.jsp.menu.MenuOperations#addMenuItem(JavaSymbolName, JavaSymbolName, String, String, String, String, String, boolean, boolean)
   */
  String addMenuItem(JavaSymbolName menuCategoryName, JavaSymbolName menuItemId,
               String menuItemLabel, String globalMessageCode, String link,
               String idPrefix, String roles, boolean hide, boolean writeProps);
   ...
   * Remove a menu entry.
   * <p>
   * If <code>menu entry</code> has children this operation will fail.
   * <p>
   * If <code>menu entry</code> has set <code>rooId</code> property this operation 
   * will only set to hidden.<br>
   *
   * @param menu entry to remove
   * @param force remove including its children
   */
  void removeEntry(JavaSymbolName pageId, boolean force);
