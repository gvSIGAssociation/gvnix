...
/**
 * gvNIX menu loader Metadata Provider
 *
...
@Service
public class MenuLoaderMetadataProvider extends AbstractMenuMetadataProvider {
	...
	return MenuLoaderMetadata.getMetadataIdentiferType();
    }

    @Override
    protected String getMetadataIdentiferFinal() {
	return MenuLoaderMetadata.getMetadataIdentiferFinal();
    }
	...
	// looks for config file constant
	String configXMLFile = null;
	FieldMetadata configXMLField = MemberFindingUtils.getField(
		classDetails, new JavaSymbolName("MENU_CONFIG_FILE"));

	if (configXMLField != null) {
	    configXMLFile = configXMLField.getFieldInitializer();
	}
	if (!StringUtils.hasText(configXMLFile)){
	    configXMLFile = null;
	}

	return new MenuLoaderMetadata(physicalTypeMetadata,
		configXMLFile.replace("\"", ""));
    }

    @Override
    protected Class getAnnotationClass() {
	return GvNIXMenuLoader.class;
    }
