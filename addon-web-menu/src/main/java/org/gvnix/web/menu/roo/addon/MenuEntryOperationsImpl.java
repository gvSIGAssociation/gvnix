...
public class MenuEntryOperationsImpl implements MenuEntryOperations {
  ...
  private static Logger logger = HandlerUtils.getLogger(MenuEntryOperationsImpl.class);
  ...
  /**
   * Use to to interact with Felix to have some sort of interactive shell 
   * that allows you to issue commands to the OSGi framework
   */
  @Reference private ShellService shellService;

  /**
   * Use to interact with the OSGi execution context including locating services
   * by reference name
   */
  ...
  public void addMenuItem(JavaSymbolName menuCategoryName,
    ...
    addMenuItem(menuCategoryName, menuItemId, menuItemLabel, globalMessageCode,
        link, idPrefix, null, false, true);
  }
  ...
  public String addMenuItem(JavaSymbolName menuCategoryName,
    ...
    if(!rootElement.getNodeName().equals("gvnix-menu")) {
      throw new IllegalArgumentException("menu.xml hasn't valid XML structure.");
    }

    // build category name and Id:
    // - menuCategoryName is a name if it doesn't start with c_: create the id
    // - menuCategoryName is an identifier if it starts with c_: create the name
    String categoryName = menuCategoryName.getSymbolName();
    StringBuilder categoryId = new StringBuilder();

    if(!categoryName.startsWith(MenuEntryOperations.CATEGORY_MENU_ITEM_PREFIX)) {
      // create categoryId using the category name
      categoryId.append(MenuEntryOperations.CATEGORY_MENU_ITEM_PREFIX).append(
    ...
    Element category = XmlUtils.findFirstElement(
        "//*[@id='".concat(categoryId.toString()).concat("']"), rootElement);
    ...
    if (category == null) {
      ...
      category = (Element) rootElement.appendChild(new XmlElementBuilder(
          ...
          .addAttribute("labelCode", categoryLabelCode)
          .build());

      properties.put(categoryLabelCode, menuCategoryName.getReadableSymbolName());
    }

    // build menu item Id:
    // - if menu item id starts with 'i_', it is a valid ID but we remove 'i_' for convenience
    ...
    // Note that in view files, menu item ID has idPrefix_, but it doesn't have
    // at application.properties, so we have to add idPrefix_ to look for 
    // the given menu item but we have to add without idPrefix_ to application.properties
    Element menuItem = XmlUtils.findFirstElement("//*[@id='"
        .concat(idPrefix)
        .concat(itemId.toString()).concat("']"), rootElement);

    String itemLabelCode = "menu_item_".concat(itemId.toString()).concat("_label");
    ...
    if (menuItem == null) {
      menuItem = new XmlElementBuilder("menu-item", document)
          ...
          .addAttribute("messageCode", StringUtils.hasText(globalMessageCode) ? globalMessageCode : "")
          .addAttribute("url", StringUtils.hasText(link) ? link : "")
          .addAttribute("hidden", Boolean.toString(hide))
          .addAttribute("roles", StringUtils.hasText(roles) ? roles : "")
          .build();
  ...
  public Document getMenuDocument() {
    ...
    if (!fileManager.exists(getMenuConfigFile())) {
      installResourceIfNeeded("/WEB-INF/views/menu.xml", "menu.xml", null,
          null);
    }

    InputStream menuIs = fileManager.getInputStream(getMenuConfigFile());
    menuDocument = org.gvnix.web.menu.roo.addon.util.XmlUtils.parseFile(menuIs);
  ...
  public void writeXMLConfigIfNeeded(Document doc) {
    ...
    if (fileManager.exists(getMenuConfigFile())) {
      ...
      Assert.notNull(mutableFile,
          "Could not create file '".concat(getMenuConfigFile()).concat("'"));
    }
   ...
   * {@inheritDoc}
   * <p>
   * Update the entry ID could change entry type because category entry
   * starts with 'c_' prefix, item entry starts with 'i_' prefix, so to
   * change a category entry to item entry you have to set a new ID that starts
   * with 'i_'.
   */
  public void updateEntry(JavaSymbolName pageId, JavaSymbolName nid,
                         String label, String messageCode, String destination,
                         String roles, Boolean hidden, boolean writeProps) {
    Document document = getMenuDocument();
    ...
    Element pageElement = XmlUtils.findFirstElement(
        "//*[@id='".concat(pageId.getSymbolName()).concat("']"), rootElement);

    // exit if menu entry doesn't exist
    Assert.notNull(pageElement, "Menu entry '".concat(pageId.getSymbolName())
        .concat("' not found. [No changes done]"));

    if(nid != null) {
      pageElement.setAttribute("id", nid.getSymbolName());

      // TODO: if Element has children, children IDs should be recalculated too
  ...
  public String getFormatedInfo(JavaSymbolName pageId, boolean label,
    ...
    Assert.notNull(pageElement, "Page '".concat(pageId.getSymbolName()).concat("' not found [No info found]"));

    // show the info of selected menu entry
    return getFormatedInfo(pageElement, label,
          messageCode, roles, lang, 0);
  }
  ...
  public void moveBefore(JavaSymbolName pageId, JavaSymbolName beforeId) {
    ...
    Element beforeElement = XmlUtils.findFirstElement(
        "//*[@id='".concat(beforeId.getSymbolName()).concat("']"), rootElement);
  ...
  public void moveInto(JavaSymbolName pageId, JavaSymbolName intoId) {
    ...
    Element intoElement = XmlUtils.findFirstElement(
        "//*[@id='".concat(intoId.getSymbolName()).concat("']"), rootElement);
  ...
  public void removeEntry(JavaSymbolName pageId, boolean force) {
    Assert.notNull(pageId, "Page ID required");
    ...
    // exit if menu entry doesn't exist
    if(pageElement == null) {
      logger.log(Level.SEVERE, "Menu entry '".concat(pageId.getSymbolName())
        .concat("' not found. [No changes done]"));
      return;
    }

    // exit if menu entry has children and force not setted
    if(org.gvnix.web.menu.roo.addon.util.XmlUtils.hasChildNodes(pageElement,
            "menu-item") && !force) {
      logger.log(Level.SEVERE, "Menu entry '".concat(pageId.getSymbolName()).concat(
            "' has children. Use --force to delete all. [No changes done]"));
      return;
    }

    // delete menu entry menu entry
    Element parent = (Element) pageElement.getParentNode();
    parent.removeChild(pageElement);
  ...
  public String getMenuConfigFile() {
    ...
    if (menuConfigFile == null) {
      menuConfigFile = getPathResolver().getIdentifier(Path.SRC_MAIN_WEBAPP,
  ...
  protected void createEntityModel(String targetPackage) {
    ...
    installIfNeeded("ContextMenuStrategy.java", targetPackage, projectMetadata);
  ...
  public void createWebArtefacts(String classesPackage) {
    ...
    params.put("__TOP_LEVEL_PACKAGE__", projectMetadata.getTopLevelPackage()
    ...
    if (isSpringSecurityInstalled()) {
      installResourceIfNeeded("/WEB-INF/tags/menu/gvnixitem.tagx",
          "gvnixitem-sec.tagx", null, new String[] { "<menu:gvnixitem",
              " xmlns:spring=", " xmlns:sec=" });
    }
    else {
      installResourceIfNeeded("/WEB-INF/tags/menu/gvnixitem.tagx",
   ...
   * <p>
   * This method has been copied from Maven addon, maybe it could be refactored
   * to utility class.
   * 
   * @param targetFilename File to create. Note this method will create the file
   *          by locating a file template with the same name as target file but
   *          prefixing "-template" to file extension. For example, given
   *          {@code Menu.java} will locate {@code Menu-template.java}
   * @param projectMetadata Project metadata to obtain project info
   */
    ...
    if (!fileManager.exists(destinationFile)) {
      try {
        ...
        InputStream templateInputStream = TemplateUtils.getTemplate(getClass(),
            fileName.concat("-template").concat(".").concat(fileExt));
        ...
        // Roos transactional file handling which offers automatic rollback if an 
        // exception occurs
   ...
   * 
   * @param relativePath path relative to {@link Path.SRC_MAIN_WEBAPP} of target
   *          file
   * @param resourceName path relative to classpath of file to be copied (cannot
   *          be null)
   * @param toReplace
  ...
  private void installResourceIfNeeded(String relativePath,
    ...
    try {
      ...
      if (toReplace != null) {
        for (Entry<String, String> entry : toReplace.entrySet()) {
          sourceContents = StringUtils.replace(sourceContents, entry.getKey(),
              entry.getValue());
        }
      }
    }
    catch (IOException e) {
      throw new IllegalStateException("Unable to load file to be copied '"
          .concat(resourceName).concat("'"), e);
    }
    finally {
      try {
        resource.close();
    ...
    if (fileManager.exists(targetPath)) {
      ...
      try {
        ...
        throw new IllegalStateException("Error reading '".concat(targetPath)
            .concat("'"), e);
      }
      finally {
        try {
  ...
  private String getAbsolutePath(String packageName, String fileName,
    ...
    return projectMetadata.getPathResolver().getIdentifier(Path.SRC_MAIN_JAVA,
        packagePath.concat("/").concat(fileName));
  }

  /**
   * Convert a package name to a fully qualified package name with
   * full support for using "~" as denoting the user's top-level package.
   *
   * @param packageName
   * @param prjMetadata
   * @return
  ...
  private String getFullyQualifiedPackageName(String packageName,
    ...
    if (packageName.startsWith("~")) {
      ...
      // analyze char after ~, if it is . do nothing, else concat . between
      // topLevelPath and given package name and remove ~ at start
      if (packageName.length() > 1) {
        newPackage = (!(packageName.charAt(1) == '.') ? topLevelPath.concat(".")
            : topLevelPath).concat(packageName.substring(1));
      }
  ...
  private void createMenu() {
    ...
    try {
      Document rooMenuDoc = XmlUtils.getDocumentBuilder().parse(rooMenuInput);
      ...
      for (int i = 0; i < menuElements.getLength(); i++) {
        ...
        if (nodeName.equals("menu:category")) {
          ...
          // We have to recover original categoryName to success addMenuItem
          // execution
          JavaSymbolName categoryId = new JavaSymbolName(
              menuCategory.getAttribute("id"));
          JavaSymbolName categoryName = new JavaSymbolName(
              StringUtils.capitalize(categoryId.getSymbolName().substring(2)));
          ...
          for (int j = 0; j < menuItems.getLength(); j++) {
            ...
            // - At menu.jspx there isn't label, no prob because it was added to
        ...
        // item elements must be inside a category
          ...
          logger.log(
              Level.SEVERE,
              "Found menu:item '".concat(menuItemId.getSymbolName()).concat(
                  "' in menu:menu tag. It must be in "
                      .concat("menu:category tag [Ignored]")));
        }
        else {
          logger.warning("Found unknow element in menu:menu tag: '".concat(
              nodeName).concat("' [Ignored]"));
        }
  ...
  private String getFormatedInfo(NodeList nodes, boolean label,
    ...
    for(int i = 0; i < nodes.getLength(); i++) {
      ...
      builder.append(getFormatedInfo((Element) node, label, messageCode, roles, 
          lang, tabSize));
    }
   ...
   * <p>
   * TODO: I think compact info better. See section "List menu structure"
   * at "docs/pd-addon-web-menu.rst". Note, it means we should refactor this
  ...
  private String getFormatedInfo(Element element, boolean label,
                                boolean message, boolean roles, I18n lang, 
                                int tabSize) {
    StringBuilder builder = new StringBuilder();
    ...
    if(label) {
      ...
      if(StringUtils.hasText(labelCode)) {
        labelValue = propFileOperations.getProperty(Path.SRC_MAIN_WEBAPP,
    ...
    if(message) {
      ...
      if(StringUtils.hasText(messageCode)) {
        ...
        if(lang != null) {
          String messageBundle = "/WEB-INF/i18n/messages_".concat(lang.getLocale().toString()).concat(".properties");
          messageValue = propFileOperations.getProperty(Path.SRC_MAIN_WEBAPP,
      ...
      builder.append(indent).append("Message Code : ").append(element.getAttribute("messageCode")).append("\n");
    ...
    if(element.hasChildNodes()) {
      ...
      builder.append(getFormatedInfo(element.getChildNodes(), label, message, 
          roles, lang, tabSize+15)); // indent to the right of "Children     : " (length 15 chars)
    }
    else {
      builder.append("\n"); // empty line      
    }
  ...
  private void updateDependencies(Element configuration) {
    ...
    List<Element> securityDependencies = XmlUtils.findElements(
        "/configuration/gvnix/menu/dependencies/dependency", configuration);
   ...
   * with "fi_", otherwise return MenuOperations.DEFAULT_MENU_ITEM_PREFIX
   */
