...
@Service
public class MenuEntryCommands implements CommandMarker { //all command types must implement the CommandMarker interface

  private static Logger logger = HandlerUtils.getLogger(MenuEntryCommands.class);
   ...
   * {@code menu setup} command in the Roo shell or directly in the
   * project.
   * 
   * @return true (default) if the command should be visible at this stage,
   *         false otherwise
  ...
  @CliAvailabilityIndicator({ "menu entry add", "menu entry visibility", 
    "menu entry remove", "menu entry move", "menu entry update", 
    "menu entry info", "menu entry roles", "menu tree" })
  public boolean areMenuCommandsAvailable() {
    return operations.isProjectAvailable() && operations.isGvNixMenuAvailable();
  }
  ...
  public String addEntry(@CliOption(key = "label", mandatory = true, help = "Text to show in menu if no messageCode set, otherwise label is used as message argument.") String label,
    ...
    String pageId = operations.addMenuItem(categoryName, new JavaSymbolName(
        StringUtils.underscoreAllWhitespace(label)), label, messageCode, url,
        null, roles, false, true);
    return "New page '".concat(pageId).concat("' added.");
  }

  @CliCommand(value = "menu entry remove", help = "Remove a menu entry. It won't delete the related view artefacts: controller, jsps, etc. If it has sub-items operation will be canceled. You can use --force to force operation and delete the children too. This operation cannot be undone.")
  public void removeEntry(@CliOption(key = "id", mandatory = true, help = "Menu entry id to remove. Use 'menu tree' to get all ids.") MenuEntry menuEntryId,
    ...
    operations.removeEntry(menuEntryId.getJavaSymbolName(), force);
  }
  ...
  public void hideEntry(@CliOption(key = "id", mandatory = true, help = "Menu entry id to show/hide. Use 'menu tree' to get all pages ids.") MenuEntry menuEntryId,
    ...
    operations.updateEntry(menuEntryId.getJavaSymbolName(), null, null, null, null, null, hidden, false);
  }
  ...
  public void secureEntry(@CliOption(key = "id", mandatory = true, help = "Menu entry ID to update.") MenuEntry menuEntryId,
  ...
  public void moveEntry(@CliOption(key = "id", mandatory = true, help = "Item to move") MenuEntry menuEntryId,
    ...
    if(intoEntryId != null && beforeEntryId != null) {
      logger.log(Level.SEVERE, "Only one of 'target' parameters can be set.");
      return;
    }
    if(intoEntryId != null && !intoEntryId.getJavaSymbolName()
        .toString().startsWith(MenuEntryOperations.CATEGORY_MENU_ITEM_PREFIX)){
      logger.log(Level.SEVERE, "An item must be moved into category entry only.");
      return;
    }
  ...
  public void updateEntry(@CliOption(key = "id", mandatory = true,  help = "Menu entry id to update. Use 'menu tree' to get all pages ids.") MenuEntry menuEntryId,
    ...
    operations.updateEntry(menuEntryId.getJavaSymbolName(), pageName, label,
        messageCode, url, roles, hidden, true);
  }

  @CliCommand(value = "menu tree", help = "List current menu tree structure.")
  public String list(@CliOption(key = "id", mandatory = false, help = "Menu entry id to show its tree structure. Default show all entries.") MenuEntry menuEntryId,
    ...
    return operations.getFormatedInfo(
        menuEntryId != null ? menuEntryId.getJavaSymbolName() : null, label,
        messageCode, roles, lang);
  }

  @CliCommand(value = "menu entry info", help = "Shows all information about a menu entry.")
  public String entryInfo(@CliOption(key = "id", mandatory = true, help = "Menu entry identifier to show info.") MenuEntry menuEntryId, 
                          @CliOption(key = "lang", mandatory = false, help = "Show messages in this language.") I18n lang) {
    return operations.getFormatedInfo(menuEntryId.getJavaSymbolName(), lang);
  }
}
