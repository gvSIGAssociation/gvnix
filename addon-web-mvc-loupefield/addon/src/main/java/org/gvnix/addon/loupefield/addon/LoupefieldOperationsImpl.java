 ...
 *         <a href="http://www.dgti.gva.es">General Directorate for Information
 *         Technologies (DGTI)</a>
 * @since 1.1
 */
@Component
...
public class LoupefieldOperationsImpl implements LoupefieldOperations {
  ...
  public void setup() {
    ...
    addToLoadScripts("loupe_js_url",
        "/resources/scripts/loupefield/jquery.loupeField.ext.gvnix.js", false);
  ...
  public void setLoupeField(JavaType controller, JavaSymbolName field,
                            String additionalFields, String caption,
                            String baseFilter, String listPath, String max) {
    Validate.notNull(controller, "Controller Java Type required");
    ...
    if (!isControllerAnnotated(existingController)) {
      LOGGER.log(Level.INFO,
          "Controller ".concat(controller.getSimpleTypeName()).concat(
              " must be annoted with @GvNIXLoupeController. Use 'web mvc loupe set' to annote controller and update views."));
    }
    ...
    if (relatedEntity == null) {
      LOGGER.log(Level.INFO,
          String.format("Field '%s' could not implements Loupe Field.",
    ...
    final PluralMetadata pluralMetadata = (PluralMetadata) getMetadataService()
        .get(PluralMetadata.createIdentifier(fieldType, PhysicalTypeIdentifier
            .getPath(relatedEntity.getDeclaredByMetadataId())));
    ...
    LOGGER.log(Level.INFO,
        String.format(
            ...
            field.getSymbolName(), "scripts/loupefield/loupe-callbacks.js",
            field.getReadableSymbolName(),
            annotationValues.getFormBackingObject().getSimpleTypeName(),
            field.getReadableSymbolName(),
            annotationValues.getFormBackingObject().getSimpleTypeName()));
  ...
  /**
   * Checks if there's a controller for the entity and if it's annotated
   * with @GvNIXDatatables with a correct mode
   *
   * @param fieldType
   */
  private void checkRelatedEntity(JavaType fieldType) {
    ...
    Set<ClassOrInterfaceTypeDetails> controllersDetails = getTypeLocationService()
        .findClassesOrInterfaceDetailsWithAnnotation(SpringJavaType.CONTROLLER);
    ...
    for (ClassOrInterfaceTypeDetails controller : controllersDetails) {
      ...
      if (rooScaffoldAnnotation != null) {
        if (fieldType.equals(rooScaffoldAnnotation
    ...
    Validate.isTrue(relatedControllers.size() != 0,
        fieldType.getSimpleTypeName().concat(
            " must have a controller annotated with @Controller and @RooWebScaffold"));
    ...
    Validate.isTrue(hasDatatablesController, fieldType.getSimpleTypeName()
    ...
    // execution
    Validate.isTrue(standardMode,
        fieldType.getSimpleTypeName()
            .concat(
                " must have a @GvNIXDatatables in any other mode than not be 'show'")
            .concat(" in order to show the loupefield correctly"));
  }

  /**
   * This method adds <code>tags/loupefield/loupe.tagx</code> to the tags folder
   */
  public void addTagx() {
    ...
    if (!getFileManager().exists(filePath)) {
      ...
      try {
        inputStream = FileUtils.getInputStream(getClass(), "tag/loupe.tagx");
        outputStream = getFileManager().createFile(filePath).getOutputStream();
  ...
  public void updateTagx() {
    ...
    try {
      ...
      if (!getFileManager().exists(filePath)) {
        ...
        outputStream = getFileManager().updateFile(filePath).getOutputStream();
      }
      IOUtils.copy(inputStream, outputStream);
    }
    catch (final IOException ioe) {
  ...
  /**
   * This method adds <code>scripts/loupefield/loupe-callbacks.js</code> to the
   * scripts folder and add to load-scripts.js to load in all pages
   */
    ...
    for (I18n i18n : supportedLanguages) {
      if (i18n.getLocale().equals(new Locale("ca"))) {
        getMessageBundleUtils().installI18nMessages(
            ...
            getFileManager());
        getMessageBundleUtils().addPropertiesToMessageBundle("ca", getClass(),
            getPropFileOperations(), getProjectOperations(), getFileManager());
        break;
      }
    }
  ...
  public void addToLoadScripts(String varName, String url, boolean isCss) {
    ...
    try {
      ...
      docTagx = XmlUtils.getDocumentBuilder()
          .parse(docTagxMutableFile.getInputStream());
    }
    catch (Exception e) {
    ...
    if (isCss) {
      modified = getWebProjectUtils().addCssToTag(docTagx, root, varName, url)
          || modified;
    }
    else {
      modified = getWebProjectUtils().addJSToTag(docTagx, root, varName, url)
          || modified;
    }
  ...
  private void doAddControllerAnnotation(JavaType controller) {
    ...
    final boolean isAlreadyAnnotated = MemberFindingUtils.getAnnotationOfType(
        existingController.getAnnotations(),
  ...
  private void setupProjectPom() {
    ...
    List<Element> properties = XmlUtils
        .findElements("/configuration/gvnix/properties/*", configuration);
  ...
  private void updateJspx(JavaType controller, String controllerPath,
    ...
    if (controllerPath != null) {
      getWebProjectUtils().addTagxUriInJspx(controllerPath, jspxName, uriMap,
          getProjectOperations(), getFileManager());
    }
  }
  ...
  private boolean existsView(String path) {
    ...
    if (!getFileManager().exists(filePath)) {
      LOGGER.log(Level.INFO,
          "View '".concat(viewFileName).concat("' doesn't exists in proyect."));
      return false;
    }
    return true;
  }
  ...
  private boolean isControllerAnnotated(ClassOrInterfaceTypeDetails controller) {
    AnnotationMetadata annotations = controller
        .getAnnotation(new JavaType(GvNIXLoupeController.class));
    if (annotations == null) {
      return false;
    }
    return true;
  ...
  private JavaType existsField(WebScaffoldAnnotationValues annotationValues,
    ...
    while (it.hasNext()) {
      ...
      if (field.getReadableSymbolName()
          .equals(currentField.getFieldName().getReadableSymbolName())) {
   ...
   */
  private boolean checkIfAdditionalFieldsExists(ClassOrInterfaceTypeDetails entity,
                                                List<? extends FieldMetadata> relatedFields,
      ...
      for (int i = 0; i < additionalFieldsList.length; i++) {
        ...
        while (it.hasNext()) {
          ...
          String relatedFieldName = StringUtils
              .uncapitalize(relatedField.getFieldName().getSymbolName());
        ...
        if (!exists) {
          LOGGER.log(Level.INFO, String.format(
              ...
              additionalFieldsList[i], entity.getName().getSimpleTypeName()));
          return false;
        }
   ...
   * @param max
   */
  private void updateViews(List<FieldMetadata> identifiers, String entityPlural,
                           String path, JavaSymbolName field,
                           String additionalFields, String caption,
                           String baseFilter, String listPath,
                           String viewName) {
    ...
    Element form = XmlUtils.findFirstElement(String.format("/div/%s", viewName),
        docRoot);
    Element element = XmlUtils
        .findFirstElement(String.format("/div/%s/*[@field='%s']", viewName,
            StringUtils.uncapitalize(field.getSymbolName())), docRoot);

    if (element == null) {
      LOGGER.log(Level.INFO,
          String.format("Could not locate field '%s' on '%s/%s'",
              StringUtils.uncapitalize(field.getSymbolName()), path, viewName));
      return;
    }
  ...
  public I18nSupport getI18nSupport() {
    if (i18nSupport == null) {
      ...
      try {
        ServiceReference<?>[] references = this.context
            .getAllServiceReferences(I18nSupport.class.getName(), null);
  ...
  public ProjectOperations getProjectOperations() {
    if (projectOperations == null) {
      ...
      try {
        ...
        LOGGER.warning(
            "Cannot load ProjectOperations on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
  ...
  public PropFileOperations getPropFileOperations() {
    if (propFileOperations == null) {
      ...
      try {
        ServiceReference<?>[] references = this.context
            .getAllServiceReferences(PropFileOperations.class.getName(), null);
        ...
        LOGGER.warning(
            "Cannot load PropFileOperations on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
  ...
  public TypeLocationService getTypeLocationService() {
    if (typeLocationService == null) {
      ...
      try {
        ServiceReference<?>[] references = this.context
            .getAllServiceReferences(TypeLocationService.class.getName(), null);
        ...
        LOGGER.warning(
            "Cannot load TypeLocationService on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
  ...
  public TypeManagementService getTypeManagementService() {
    if (typeManagementService == null) {
      ...
      try {
        ...
        LOGGER.warning(
            "Cannot load TypeManagementService on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
  ...
  public MetadataService getMetadataService() {
    if (metadataService == null) {
      ...
      try {
        ...
        LOGGER.warning(
            "Cannot load MetadataService on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
  ...
  public PersistenceMemberLocator getPersistenceMemberLocator() {
    if (persistenceMemberLocator == null) {
      ...
      try {
        ...
        LOGGER.warning(
            "Cannot load PersistenceMemberLocator on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
  ...
  public WebProjectUtils getWebProjectUtils() {
    if (webProjectUtils == null) {
      ...
      try {
        ServiceReference<?>[] references = this.context
            .getAllServiceReferences(WebProjectUtils.class.getName(), null);

        for (ServiceReference<?> ref : references) {
          webProjectUtils = (WebProjectUtils) this.context.getService(ref);
        ...
        LOGGER.warning(
            "Cannot load WebProjectUtils on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
  ...
  public MessageBundleUtils getMessageBundleUtils() {
    if (messageBundleUtils == null) {
      ...
      try {
        ServiceReference<?>[] references = this.context
            .getAllServiceReferences(MessageBundleUtils.class.getName(), null);
        ...
        LOGGER.warning(
            "Cannot load MessageBundleUtils on LoupeFieldOperationsImpl.");
        return null;
      }
    }
    else {
