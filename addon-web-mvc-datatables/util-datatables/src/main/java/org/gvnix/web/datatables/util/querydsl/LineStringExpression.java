 ...
 * @param <T>
 */
public abstract class LineStringExpression<T extends LineString> extends
        CurveExpression<T> {
    ...
    public NumberExpression<Integer> numPoints() {
        if (numPoints == null) {
            numPoints = NumberOperation.create(Integer.class,
                    SpatialOps.NUM_POINTS, mixin);
        }
        return numPoints;
    }
    ...
    public PointExpression<Point> pointN(int idx) {
        return PointOperation.create(Point.class, SpatialOps.POINTN, mixin,
                ConstantImpl.create(idx));
    }
