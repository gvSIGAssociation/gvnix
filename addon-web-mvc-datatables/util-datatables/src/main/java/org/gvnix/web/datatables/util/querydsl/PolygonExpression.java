 ...
 * interior boundaries. Each interior boundary defines a hole in the Polygon. A
 * Triangle is a polygon with 3 distinct, non-collinear vertices and no interior
 * boundary.
 * 
 * @author tiwe
 * 
 * @param <T>
 */
public abstract class PolygonExpression<T extends Polygon> extends
        SurfaceExpression<T> {
    ...
    public LineStringExpression<?> exteriorRing() {
        if (exterorRing == null) {
            exterorRing = LineStringOperation.create(LineString.class,
                    SpatialOps.EXTERIOR_RING, mixin);
        }
        return exterorRing;
    }
    ...
    public NumberExpression<Integer> numInteriorRing() {
        if (numInteriorRing == null) {
            numInteriorRing = NumberOperation.create(Integer.class,
                    SpatialOps.NUM_INTERIOR_RING, mixin);
        }
    ...
    public LineStringExpression<LineString> interiorRingN(int idx) {
        return LineStringOperation.create(LineString.class,
