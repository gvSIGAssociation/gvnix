 ...
 * @param <T>
 */
public abstract class SurfaceExpression<T extends Geometry> extends
    ...
    public PointExpression<Point> centroid() {
        if (centroid == null) {
            centroid = PointOperation.create(Point.class, SpatialOps.CENTROID,
                    mixin);
        }
        return centroid;
    }
    ...
    public PointExpression<Point> pointOnSurface() {
        if (pointOnSurface == null) {
            pointOnSurface = PointOperation.create(Point.class,
                    SpatialOps.POINT_ON_SURFACE, mixin);
        }
