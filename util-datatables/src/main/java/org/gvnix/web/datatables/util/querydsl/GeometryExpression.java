 ...
 * @param <T>
 */
public abstract class GeometryExpression<T extends Geometry> extends
        SimpleExpression<T> {

    private static final long serialVersionUID = -1183228394472681995L;

    @Nullable
    private volatile NumberExpression<Integer> dimension, coordinateDimension,
            spatialDimension, srid;
    ...
    @Nullable
    private volatile GeometryExpression<Geometry> envelope, boundary,
            convexHull;
    ...
    public NumberExpression<Integer> dimension() {
        if (dimension == null) {
            dimension = NumberOperation.create(Integer.class,
                    SpatialOps.DIMENSION, mixin);
        }
        return dimension;
    }
    ...
    public StringExpression geometryType() {
        if (geometryType == null) {
            geometryType = StringOperation.create(SpatialOps.GEOMETRY_TYPE,
                    mixin);
        }
        return geometryType;
    }
    ...
    /**
     * Returns the Spatial Reference System ID for this geometric object. This
     * will normally be a foreign key to an index of reference systems stored in
     * either the same or some other datastore.
     * 
     * @return
     */
    public NumberExpression<Integer> srid() {
        if (srid == null) {
            srid = NumberOperation
                    .create(Integer.class, SpatialOps.SRID, mixin);
        }
        return srid;
    }
     ...
     * MINY), (MAXX, MINY), (MAXX, MAXY), (MINX, MAXY), (MINX, MINY)]. Minimums
     * for Z and M may be added. The simplest representation of an Envelope is
     * as two direct positions, one containing all the minimums, and another all
    ...
    public GeometryExpression<Geometry> envelope() {
        if (envelope == null) {
            envelope = GeometryOperation.create(Geometry.class,
                    SpatialOps.ENVELOPE, mixin);
        }
        return envelope;
    }
    ...
    public SimpleExpression<byte[]> asBinary() {
        if (binary == null) {
            binary = SimpleOperation.create(byte[].class, SpatialOps.AS_BINARY,
                    mixin);
        }
        return binary;
    }
    ...
    public GeometryExpression<Geometry> boundary() {
        if (boundary == null) {
            boundary = GeometryOperation.create(Geometry.class,
                    SpatialOps.BOUNDARY, mixin);
        }
        return boundary;
    }
     ...
     * @return
     */
    public BooleanExpression relate(Expression<? extends Geometry> geometry,
            String matrix) {
        return BooleanOperation.create(SpatialOps.RELATE, mixin, geometry,
                ConstantImpl.create(matrix));
    }
     ...
     * @return
     */
    public NumberExpression<Double> distance(
            Expression<? extends Geometry> geometry) {
        return NumberOperation.create(Double.class, SpatialOps.DISTANCE, mixin,
                geometry);
    }

    // TODO maybe move out
    public NumberExpression<Double> distanceSphere(
            Expression<? extends Geometry> geometry) {
        return NumberOperation.create(Double.class, SpatialOps.DISTANCE_SPHERE,
                mixin, geometry);
    }

    // TODO maybe move out
    public NumberExpression<Double> distanceSpheroid(
            Expression<? extends Geometry> geometry) {
        return NumberOperation.create(Double.class,
                SpatialOps.DISTANCE_SPHEROID, mixin, geometry);
    }
    ...
    public GeometryExpression<Geometry> buffer(double distance) {
        return GeometryOperation.create(Geometry.class, SpatialOps.BUFFER,
                mixin, ConstantImpl.create(distance));
    }
    ...
    public GeometryExpression<Geometry> convexHull() {
        if (convexHull == null) {
            convexHull = GeometryOperation.create(Geometry.class,
                    SpatialOps.CONVEXHULL, mixin);
        }
        return convexHull;
    }
     ...
     * @return
     */
    public GeometryExpression<Geometry> intersection(
            Expression<? extends Geometry> geometry) {
        return GeometryOperation.create(Geometry.class,
                SpatialOps.INTERSECTION, mixin, geometry);
    }
     ...
     * @return
     */
    public GeometryExpression<Geometry> union(
            Expression<? extends Geometry> geometry) {
        return GeometryOperation.create(Geometry.class, SpatialOps.UNION,
                mixin, geometry);
    }
     ...
     * @return
     */
    public GeometryExpression<Geometry> difference(
            Expression<? extends Geometry> geometry) {
        return GeometryOperation.create(Geometry.class, SpatialOps.DIFFERENCE,
                mixin, geometry);
    }
     ...
     * @return
     */
    public GeometryExpression<Geometry> symDifference(
            Expression<? extends Geometry> geometry) {
        return GeometryOperation.create(Geometry.class,
                SpatialOps.SYMDIFFERENCE, mixin, geometry);
    }

    public GeometryExpression<Geometry> transform(int srid) {
        return GeometryOperation.create(Geometry.class, SpatialOps.TRANSFORM,
                mixin, ConstantImpl.create(srid));
    }
