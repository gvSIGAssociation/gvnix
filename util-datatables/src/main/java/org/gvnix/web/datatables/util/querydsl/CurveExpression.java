 ...
 * @param <T>
 */
public abstract class CurveExpression<T extends Geometry> extends
    ...
    public PointExpression<Point> startPoint() {
        if (startPoint == null) {
            startPoint = PointOperation.create(Point.class,
                    SpatialOps.START_POINT, mixin);
        }
        return startPoint;
    }
    ...
    public PointExpression<Point> endPoint() {
        if (endPoint == null) {
            endPoint = PointOperation.create(Point.class, SpatialOps.END_POINT,
                    mixin);
        }
        return endPoint;
    }
