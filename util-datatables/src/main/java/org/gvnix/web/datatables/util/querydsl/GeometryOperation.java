 ...
 * @param <T>
 */
public class GeometryOperation<T extends Geometry> extends
        GeometryExpression<T> implements Operation<T> {

    private static final long serialVersionUID = 3433471874808633698L;

    public static <D extends Geometry> GeometryOperation<D> create(
            Class<D> type, Operator<? super D> op, Expression<?> one) {
        return new GeometryOperation<D>(type, op,
    ...
    protected GeometryOperation(Class<T> type, Operator<? super T> op,
            Expression<?>... args) {
