...
public class AddonOperationsImpl implements AddonOperations {
	...
	public void annotateType(JavaType javaType) {
		...
		if (MemberFindingUtils.getAnnotationOfType(mutableTypeDetails.getAnnotations(), new JavaType(RooAddon.class.getName())) == null) {
	...
	public void setup() {
		...
		projectOperations.addRepository(new Repository("Addon Roo add-on repository", "Addon Roo add-on repository", "https://gvnix - spring roo - addon - web mvc screen patterns.googlecode.com/svn/repo"));
		
		List<Dependency> dependencies = new ArrayList<Dependency>();
		
		// Install the dependency on the add-on jar (
		dependencies.add(new Dependency("org.gvnix.web.screen.roo.addon", "org.gvnix.web.screen.roo.addon", "0.1.0.BUILD-SNAPSHOT", DependencyType.JAR, DependencyScope.PROVIDED));
