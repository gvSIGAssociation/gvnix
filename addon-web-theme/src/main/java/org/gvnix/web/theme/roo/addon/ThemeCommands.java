...
@Service
public class ThemeCommands implements CommandMarker { //all command types must implement the CommandMarker interface
  ...
  public void installTheme(@CliOption(key = "id", mandatory = true, help = "Theme ID available to be installed. Use the command 'theme list' to see which are available to install") Theme theme) {

    // Don't use Assert to avoid stack trace in shell, use logger
    if(theme == null) {
      logger.log(Level.SEVERE, "Theme not found. Use the command 'theme list' to see which are available to install.");
      return;
    }
  ...
  public void setActiveTheme(@CliOption(key = "id", mandatory = true, optionContext = "INSTALLED", help = "Theme's ID installed in theme's project directory. Use the command 'theme list' to see which are installed") Theme theme) {

    // Don't use Assert to avoid stack trace in shell, use logger
    if(theme == null) {
      logger.log(Level.SEVERE, "Theme doesn't exist in project themes folder. Use the command 'theme list' to see which are available to active.");
      return;
    }
