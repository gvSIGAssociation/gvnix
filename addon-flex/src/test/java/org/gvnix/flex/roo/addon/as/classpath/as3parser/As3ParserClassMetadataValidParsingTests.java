...
import org.gvnix.flex.roo.addon.as.classpath.as3parser.As3ParserMutableClassOrInterfaceTypeDetails;
...
public class As3ParserClassMetadataValidParsingTests {
    ...
    public void setUp() throws Exception {
        ...
        details = (As3ParserMutableClassOrInterfaceTypeDetails) metadata
    ...
    public void testPackageAndClassDetails() {
        ...
        assertEquals("com.foo.stuff", details.getName().getPackage()
                .getFullyQualifiedPackageName());
        assertEquals(details.getCompilationUnitPackage(), details.getName()
                .getPackage());
        assertEquals("com.foo.stuff.FooImpl", details.getName()
                .getFullyQualifiedTypeName());
        assertEquals("FooImpl", details.getName().getSimpleTypeName());
        assertEquals(ASPhysicalTypeCategory.CLASS,
                details.getPhysicalTypeCategory());
        ...
        assertEquals("com.foo.Foo", details.getImplementsTypes().get(0)
                .getFullyQualifiedTypeName());
        assertEquals(1, details.getExtendsTypes().size());
        assertEquals("com.foo.Alpha", details.getExtendsTypes().get(0)
                .getFullyQualifiedTypeName());
    }
    ...
    public void testClassLevelMetadata() {
        ...
        assertEquals("ClassLevelTag1", details.getTypeMetaTags().get(0)
                .getName());
        assertEquals(0, details.getTypeMetaTags().get(0).getAttributeNames()
                .size());
        assertEquals("ClassLevelTag2", details.getTypeMetaTags().get(1)
                .getName());
        assertEquals(1, details.getTypeMetaTags().get(1).getAttributeNames()
                .size());
        assertEquals(
                "bar",
                details.getTypeMetaTags().get(1)
                        .getAttribute(new ActionScriptSymbolName("foo"))
                        .getValue());
    }
    ...
    public void testImportedTypeField() {
        ...
        assertEquals("Field2", field.getFieldName()
                .getSymbolNameCapitalisedFirstLetter());
        assertEquals(ASTypeVisibility.PRIVATE, field.getVisibility());
        assertEquals("com.foo.Bar", field.getFieldType()
                .getFullyQualifiedTypeName());
    ...
    public void testPublicFactoryMethod() {
        ...
        assertEquals(0, method.getParameterNames().size());
        assertEquals("com.foo.stuff.FooImpl", method.getReturnType()
                .getFullyQualifiedTypeName());
    ...
    public void testPublicVoidMethodWithParams() {
        ...
        assertEquals("String", method.getParameterTypes().get(0)
                .getFullyQualifiedTypeName());
        assertEquals("baz", method.getParameterNames().get(1).getSymbolName());
        assertEquals("com.foo.Bar", method.getParameterTypes().get(1)
                .getFullyQualifiedTypeName());
