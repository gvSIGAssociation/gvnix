...
import org.gvnix.flex.roo.addon.as.classpath.as3parser.As3ParserClassMetadata;
import org.gvnix.flex.roo.addon.as.classpath.as3parser.As3ParserMetadataProvider;
import org.gvnix.flex.roo.addon.as.classpath.details.ASConstructorMetadata;
import org.gvnix.flex.roo.addon.as.classpath.details.ASFieldMetadata;
import org.gvnix.flex.roo.addon.as.classpath.details.ASMethodMetadata;
import org.gvnix.flex.roo.addon.as.classpath.details.DefaultASClassOrInterfaceTypeDetails;
import org.gvnix.flex.roo.addon.as.classpath.details.DefaultASConstructorMetadata;
import org.gvnix.flex.roo.addon.as.classpath.details.DefaultASFieldMetadata;
import org.gvnix.flex.roo.addon.as.classpath.details.DefaultASMethodMetadata;
...
public class As3ParserMetadataProviderTests {
    ...
    private static final Log log = LogFactory
            .getLog(As3ParserMetadataProviderTests.class);
    ...
    public void setUp() throws Exception {
        ...
        metadataId = "MID:" + ASPhysicalTypeIdentifier.class.getName()
                + "#SRC_MAIN_FLEX?com.foo.stuff.FooImpl";
        ...
        ReflectionTestUtils.setField(provider, "metadataService",
                metadataService);
        ReflectionTestUtils.setField(provider, "metadataDependencyRegistry",
                registry);
    ...
    public void testGetWithValidIdentifier() {
        As3ParserClassMetadata metadata = (As3ParserClassMetadata) provider
                .get(metadataId);
    ...
    public void testCreatePhysicalType_Class() throws IOException {
        String fileIdentifier = new ClassPathResource("").getFile()
                .getCanonicalPath() + "com/foo/stuff/FooImpl.as";
        ASPhysicalTypeDetails details = new DefaultASClassOrInterfaceTypeDetails(
                metadataId, new ActionScriptType("com.foo.stuff.FooImpl"),
                ASPhysicalTypeCategory.CLASS, null);
        ASPhysicalTypeMetadata type = new DefaultASPhysicalTypeMetadata(
                metadataId, fileIdentifier, details);
        provider.createPhysicalType(type);
        assertTrue(StringUtils.isNotBlank(fileManager.lastFile));

        ASCompilationUnit compUnit = factory.newParser().parse(
                new StringReader(fileManager.lastFile));
    ...
    public void testCreatePhysicalType_Interface() throws IOException {
        String fileIdentifier = new ClassPathResource("").getFile()
                .getCanonicalPath() + "com/foo/stuff/FooImpl.as";
        ASPhysicalTypeDetails details = new DefaultASClassOrInterfaceTypeDetails(
                metadataId, new ActionScriptType("com.foo.stuff.Foo"),
    ...
    @Test
    public void testCreatePhysicalType_ClassWithTypeMetaTag()
            throws IOException {
        String fileIdentifier = new ClassPathResource("").getFile()
                .getCanonicalPath() + "com/foo/stuff/FooImpl.as";
    ...
    public void testCreatePhysicalType_ClassWithSuperclass() throws IOException {
        ...
        ASPhysicalTypeDetails details = new DefaultASClassOrInterfaceTypeDetails(
                metadataId, new ActionScriptType("com.foo.stuff.FooImpl"),
                ASPhysicalTypeCategory.CLASS, null, null, null, null,
                extendsTypes, null, null);
        ASPhysicalTypeMetadata type = new DefaultASPhysicalTypeMetadata(
                metadataId, fileIdentifier, details);
        provider.createPhysicalType(type);
        assertTrue(StringUtils.isNotBlank(fileManager.lastFile));
        ...
        assertTrue(compUnit.getPackage().findImports()
                .contains("com.foo.other.Parent"));
    }
    ...
    public void testCreatePhysicalType_ClassWithInterfaces() throws IOException {
        ...
        ASPhysicalTypeDetails details = new DefaultASClassOrInterfaceTypeDetails(
                metadataId, new ActionScriptType("com.foo.stuff.FooImpl"),
                ASPhysicalTypeCategory.CLASS, null, null, null, null, null,
        ...
        assertTrue(compUnit.getPackage().findImports()
                .contains("com.foo.other.Parent"));
        assertTrue(compUnit.getPackage().findImports()
                .contains("com.foo.other.Sibling"));
    }

    @Test
    public void testCreatePhysicalType_ClassWithSimpleConstructor()
            throws IOException {
        String fileIdentifier = new ClassPathResource("").getFile()
                .getCanonicalPath() + "com/foo/stuff/FooImpl.as";

        ASConstructorMetadata constructor = new DefaultASConstructorMetadata(
                metadataId);

        ASPhysicalTypeDetails details = new DefaultASClassOrInterfaceTypeDetails(
                metadataId, new ActionScriptType("com.foo.stuff.FooImpl"),
                ASPhysicalTypeCategory.CLASS, null, constructor, null, null,
                null, null, null);
        ASPhysicalTypeMetadata type = new DefaultASPhysicalTypeMetadata(
                metadataId, fileIdentifier, details);
        provider.createPhysicalType(type);
    ...
    @Test
    public void testCreatePhysicalType_ClassWithComplexConstructor()
            throws IOException {
        String fileIdentifier = new ClassPathResource("").getFile()
                .getCanonicalPath() + "com/foo/stuff/FooImpl.as";

        List<ASMetaTagMetadata> metaTags = new ArrayList<ASMetaTagMetadata>();
        ASMetaTagMetadata metaTag = new DefaultASMetaTagMetadata("Autowired",
                null);
        ...
        ASConstructorMetadata constructor = new DefaultASConstructorMetadata(
                metadataId, null, metaTags, paramTypes, paramNames,
                ASTypeVisibility.PRIVATE);
        ...
        assertTrue(compUnit.getPackage().findImports()
                .contains("com.foo.other.Bar"));
    }

    @Test
    public void testCreatePhysicalType_ClassWithSimpleMethod()
            throws IOException {
        String fileIdentifier = new ClassPathResource("").getFile()
                .getCanonicalPath() + "com/foo/stuff/FooImpl.as";

        List<ASMethodMetadata> methods = new ArrayList<ASMethodMetadata>();
        ASMethodMetadata method = new DefaultASMethodMetadata(metadataId,
                new ActionScriptSymbolName("doStuff"),
                ActionScriptType.VOID_TYPE, ASTypeVisibility.PUBLIC);
        methods.add(method);

        ASPhysicalTypeDetails details = new DefaultASClassOrInterfaceTypeDetails(
                metadataId, new ActionScriptType("com.foo.stuff.FooImpl"),
                ASPhysicalTypeCategory.CLASS, null, null, methods, null, null,
                null, null);
        ASPhysicalTypeMetadata type = new DefaultASPhysicalTypeMetadata(
                metadataId, fileIdentifier, details);
        provider.createPhysicalType(type);
    ...
    @Test
    public void testCreatePhysicalType_ClassWithComplexMethod()
            throws IOException {
        String fileIdentifier = new ClassPathResource("").getFile()
                .getCanonicalPath() + "com/foo/stuff/FooImpl.as";

        List<ASMetaTagMetadata> metaTags = new ArrayList<ASMetaTagMetadata>();
        ASMetaTagMetadata metaTag = new DefaultASMetaTagMetadata(
                "MagicalMetadata", null);
        metaTags.add(metaTag);
        ...
        List<ASMethodMetadata> methods = new ArrayList<ASMethodMetadata>();
        ASMethodMetadata method = new DefaultASMethodMetadata(metadataId,
                new ActionScriptSymbolName("doStuff"), new ActionScriptType(
                        "com.foo.smothered.Covered"), ASTypeVisibility.PRIVATE,
                "", metaTags, paramTypes, paramNames);
        methods.add(method);
        ...
        assertTrue(compUnit.getPackage().findImports()
                .contains("com.foo.other.Bar"));
        assertTrue(compUnit.getPackage().findImports()
                .contains("com.foo.smothered.Covered"));
    }
    ...
    public void testCreatePhysicalType_ClassWithFields() throws IOException {
        ...
        List<ASMetaTagMetadata> metaTags = new ArrayList<ASMetaTagMetadata>();
        ASMetaTagMetadata bindableTag = new DefaultASMetaTagMetadata(
                "Bindable", null);
        metaTags.add(bindableTag);

        List<ASFieldMetadata> fields = new ArrayList<ASFieldMetadata>();
        ASFieldMetadata field1 = new DefaultASFieldMetadata(metadataId,
                new ActionScriptType("String"), new ActionScriptSymbolName(
                        "name"), null, null, metaTags);
        ASFieldMetadata field2 = new DefaultASFieldMetadata(metadataId,
                new ActionScriptType("com.foo.other.Brother"),
                new ActionScriptSymbolName("brother"),
                ASTypeVisibility.PROTECTED, null, null);
        ASFieldMetadata field3 = new DefaultASFieldMetadata(metadataId,
                new ActionScriptType("com.foo.stuff.Stuff"),
                new ActionScriptSymbolName("stuff"), ASTypeVisibility.PRIVATE,
                null, null);
        fields.add(field1);
        fields.add(field2);
        fields.add(field3);
        ...
        ASPhysicalTypeDetails details = new DefaultASClassOrInterfaceTypeDetails(
                metadataId, new ActionScriptType("com.foo.stuff.FooImpl"),
                ASPhysicalTypeCategory.CLASS, fields, null, null, null, null,
                null, null);
        ASPhysicalTypeMetadata type = new DefaultASPhysicalTypeMetadata(
                metadataId, fileIdentifier, details);
        provider.createPhysicalType(type);
        ...
        assertTrue(compUnit.getPackage().findImports()
                .contains("com.foo.other.Brother"));
        assertFalse(compUnit.getPackage().findImports()
                .contains("com.foo.stuff.Stuff"));
    }

    // @Test
    public void testFindIdentifier() {

        String expectedId = "MID:" + ASPhysicalTypeIdentifier.class.getName()
                + "#SRC_MAIN_FLEX?com.foo.stuff.FooImpl";
        String id = provider.findIdentifier(new ActionScriptType(
                "com.foo.stuff.FooImpl"));
    ...
    public void testOnFileEvent() throws IOException {
        ...
        FileDetails details = new FileDetails(new ClassPathResource(
                "com/foo/stuff/FooImpl.as").getFile(), new Date().getTime());
